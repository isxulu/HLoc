...
class FeaturePairsDataset(torch.utils.data.Dataset):
    """
        为匹配对定义数据集,继承自torch的Dataset
    """

    def __init__(self, pairs, feature_path_q, feature_path_r):
        """
            初始化,记录图像对,和两组特征路径,分别对应着pair中的names_q names_ref
        """
        self.pairs = pairs
    ...
    def __getitem__(self, idx):
        """
            ...
            返回值：存有两张图像特征信息的字典
        """
        name0, name1 = self.pairs[idx]  # 第idx组匹配对的两张图像名称
        data = {}

        # 读取name0这张图片的特征，并存在字典中
...
def match_from_paths(conf: Dict,
    ...
    pairs = [(q, r) for q, rs in pairs.items() for r in rs]  # 将匹配的对一次写入pairs列表
    pairs = find_unique_new_pairs(pairs, None if overwrite else match_path)  # 去重，节省时间
    if len(pairs) == 0:
